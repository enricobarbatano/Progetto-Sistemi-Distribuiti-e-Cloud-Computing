...
// Il nodo espone tramite gRPC due servizi principali:
//
...
func main() {
	// Lettura della configurazione del nodo.
	// Se una variabile d'ambiente non è definita, viene usato un valore di default.
	nodeID := getEnv("NODE_ID", "node-1")
	nodeAddress := getEnv("NODE_ADDRESS", "localhost:50051")
	port := getEnv("PORT", "50051")
	dataDir := getEnv("DATA_DIR", "data")

	// Parsing della lista dei peer.
	// La mappa risultante associa l'id logico di un nodo al suo indirizzo gRPC.
	peers := parsePeers(os.Getenv("PEERS"))

	// Creazione della struttura interna del Consensus Node.
	// Qui vengono inizializzati stato Raft, storage key-value locale,
	// metadati del nodo e persistenza su disco.
	...
	// gRPC gestirà automaticamente le richieste in arrivo usando goroutine.
	server := grpc.NewServer()

	// Registrazione del servizio di consenso.
	// In questo modo il nodo può ricevere RPC come RequestVote,
	// AppendEntries e InstallSnapshot.
	consensuspb.RegisterConsensusServiceServer(server, node)

	// Registrazione del servizio key-value.
	// In questo modo lo stesso nodo può ricevere RPC come Put, Get,
	// Delete e GetLeader.
	...
	// Questa chiamata è bloccante: il processo resta in ascolto finché
	// il server non viene fermato o si verifica un errore.
...
// default passato come parametro. Questa funzione evita di ripetere lo stesso
// controllo più volte nel main.
...
//
// Le entry malformate vengono ignorate, così un errore nella lista dei peer
// non causa immediatamente il crash del nodo in questa fase iniziale.
func parsePeers(raw string) map[string]string {
	peers := make(map[string]string)

	// Se non sono stati specificati peer, il nodo parte comunque.
	// Questo è utile per testare un singolo nodo localmente.
	if raw == "" {
		return peers
	}

	// Ogni peer è separato da virgola.
	parts := strings.Split(raw, ",")
	for _, part := range parts {
		// Ogni peer deve avere il formato id=address.
		...
		// Inseriamo il peer solo se sia id sia address sono valorizzati.
		if id != "" && address != "" {
			peers[id] = address
		}
	}
